...
const ManagementBlogEditOverviewContentEditorCard = (props: Props) => {
  // ** Props
  const { initBlogEntity } = props

  // ** States
  const [blocks, setBlocks] = useState<OutputData | undefined>(initBlogEntity.content || undefined)
  const [isEditMode, setIsEditMode] = useState<boolean>(false)
  const [editorInstance, setEditorInstance] = useState<EditorCore | null>(null)

  // ** Hooks
  const [updateBlog, { isLoading: isUpdateBlogLoading }] = useUpdateOneMutation()
...
export default ManagementBlogEditOverviewContentEditorCard
