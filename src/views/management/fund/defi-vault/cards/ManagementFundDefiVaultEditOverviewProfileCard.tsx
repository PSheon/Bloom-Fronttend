...
import ManagementFundDefiVaultEditBannerPreviewBox from 'src/views/management/fund/defi-vault/boxes/ManagementFundDefiVaultEditBannerPreviewBox'
...
const schema = yup.object().shape({
  ...
  defaultReferrerAddress: yup.string().optional(),
...
const ManagementFundDefiVaultEditOverviewProfileCard = (props: Props) => {
  // ** Props
  ...
  // ** Hooks
  const [updateDVFund, { data: updatedDVFund = initDVFundEntity, isLoading: isUpdateDVFundLoading }] =
  ...
  const {
    ...
    defaultValues: {
      displayName: initDVFundEntity.displayName,
      description: initDVFundEntity.description || '',
      saleStartTime: new Date(initDVFundEntity.saleStartTime),
      maturityDate: new Date(initDVFundEntity.maturityDate),
      estimatedAPY: initDVFundEntity.estimatedAPY,
      performanceFeePercentage: initDVFundEntity.performanceFeePercentage,
      redemptionFrequencyInDays: initDVFundEntity.redemptionFrequencyInDays,
      defaultReferrerAddress: initDVFundEntity.defaultReferrerAddress || '',
      discordUrl: initDVFundEntity.discordUrl || '',
      twitterUrl: initDVFundEntity.twitterUrl || ''
    },
    mode: 'onBlur',
  ...
  // ** Vars
  const statusProperties = getFundStatusProperties(initDVFundEntity.status)
  ...
  return (
    <Card>
      <CardContent sx={{ display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
        <ManagementFundDefiVaultEditBannerPreviewBox initDVFundEntity={updatedDVFund} />
      ...
      <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
        ...
        <Stack spacing={2.7}>
          <Stack direction='row' flexWrap='wrap' spacing={2} alignItems='center'>
            ...
            <Typography variant='body2'>{initDVFundEntity.description || '未說明'}</Typography>
          </Stack>
          <Stack direction='row' spacing={2} alignItems='center'>
            <Typography variant='subtitle2' color='text.primary'>
              創始日期：
            </Typography>
            <Typography variant='body2'>{format(new Date(initDVFundEntity.genesisDate), 'PPpp')}</Typography>
          </Stack>
          <Stack direction='row' spacing={2} alignItems='center'>
            <Typography variant='subtitle2' color='text.primary'>
              結束日期：
            </Typography>
            <Typography variant='body2'>{format(new Date(initDVFundEntity.maturityDate), 'PPpp')}</Typography>
          </Stack>
          <Stack direction='row' spacing={2} alignItems='center'>
            <Typography variant='subtitle2' color='text.primary'>
              預期APY：
            </Typography>
            <Typography variant='body2'>{`${initDVFundEntity.estimatedAPY}%`}</Typography>
          </Stack>
          <Stack direction='row' spacing={2} alignItems='center'>
            <Typography variant='subtitle2' color='text.primary'>
              績效手續費：
            </Typography>
            <Typography variant='body2'>{`${initDVFundEntity.performanceFeePercentage}%`}</Typography>
          </Stack>
          <Stack direction='row' spacing={2} alignItems='center'>
            <Typography variant='subtitle2' color='text.primary'>
              協議手續費：
          ...
          <Stack direction='row' spacing={2} alignItems='center'>
            ...
            <Typography variant='body2'>{format(new Date(initDVFundEntity.saleStartTime), 'PPpp')}</Typography>
          </Stack>
          <Stack direction='row' spacing={2} alignItems='center'>
            <Typography variant='subtitle2' color='text.primary'>
              兌換週期：
            ...
            <Typography variant='body2'>
              {initDVFundEntity.redemptionFrequencyInDays
                ? `${initDVFundEntity.redemptionFrequencyInDays} 天`
                : '未設定'}
            </Typography>
          </Stack>
        </Stack>
                ...
                <Grid item xs={12}>
                  <FormControl fullWidth>
                    <TextField
                      fullWidth
                      label='創始日期'
                      value={format(new Date(initDVFundEntity.genesisDate), 'PPpp')}
                    ...
                    <Controller
                      ...
                      render={({ field: { value, onChange } }) => (
                        <DatePicker
                          ...
                          timeFormat='HH:mm'
                          timeIntervals={15}
                          minDate={new Date(initDVFundEntity.genesisDate)}
                          maxDate={new Date(initDVFundEntity.maturityDate)}
                          onChange={(date: Date) => {
                            onChange(date)
                          }}
                    ...
                    <Controller
                      ...
                      render={({ field: { value, onChange, onBlur } }) => (
                        <TextField
                          ...
                          error={Boolean(errors.defaultReferrerAddress)}
                        />
                      )}
                    />
                    {errors.defaultReferrerAddress && (
                      <FormHelperText sx={{ color: 'error.main' }} id='fund-edit-default-referrer-address'>
                        {errors.defaultReferrerAddress.message}
                      </FormHelperText>
                    )}
                  </FormControl>
                </Grid>
...
export default ManagementFundDefiVaultEditOverviewProfileCard
